	...
	static public void Load (BMFont font, string name, byte[] bytes)
	{
		...
		if (bytes != null)
		{
			...
			while (reader.canRead)
			{
                ...
                {
                    // Expected data style:
                    // info face="FZCuYuan-M03" size=38 bold=0 italic=0 charset="" unicode=0 stretchH=100 smooth=1 aa=1 padding=1,1,1,1 spacing=2,2
                    font.fontName   = GetString(split[1]);
                    font.fontSize   = GetInt(split[2]);
                }
