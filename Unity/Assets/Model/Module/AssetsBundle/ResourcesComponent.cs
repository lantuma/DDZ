...
namespace ETModel
{
	...
	public static class AssetBundleHelper
	{
		...
		public static void CollectDependencies(List<string> parents, string assetBundleName, Dictionary<string, int> info)
		{
			...
			foreach (string dep in deps)
			{
				...
				{
                    Debug.LogError($"包有循环依赖，请重新标记: {assetBundleName} {dep}");
	...
	public class ResourcesComponent : Component
	{
        ...
        //AB包图片缓冲集合
        public readonly Dictionary<string, Dictionary<string, Sprite>> DicSpricteCahe = new Dictionary<string, Dictionary<string, Sprite>>();
        ...
        public UnityEngine.Object GetAsset(string bundleName, string prefab)
		{
			...
			{
                Debug.LogError($"not found asset: {bundleName} {prefab}");
        ...
        public UnityEngine.Sprite GetSprite(string bundleName,string prefab)
        {
            Dictionary<string, UnityEngine.Sprite> dict;
            if (!this.DicSpricteCahe.TryGetValue(bundleName.BundleNameToLower(), out dict))
            {
                Debug.LogError($"not found asset: {bundleName} {prefab}");
                throw new Exception($"not found asset: {bundleName} {prefab}");
            }
            ...
            if (!dict.TryGetValue(prefab, out sprite))
            {
                Debug.LogError($"not found asset: {bundleName} {prefab}");
                throw new Exception($"not found asset: {bundleName} {prefab}");
            }
		...
		private void UnloadOneBundle(string assetBundleName)
		{
			...
			{
                Debug.LogError( $"not found assetBundle: {assetBundleName}");
        ...
        public void AddSpriteResource(string bundleName, string assetName, UnityEngine.Sprite sprite)
        {
            Dictionary<string, UnityEngine.Sprite> dict;
            if (!this.DicSpricteCahe.TryGetValue(bundleName.BundleNameToLower(), out dict))
            {
                dict = new Dictionary<string, UnityEngine.Sprite>();
                this.DicSpricteCahe[bundleName] = dict;
            }
            dict[assetName] = sprite;
        }

		public void LoadOneBundle(string assetBundleName, bool IsSprite = false)
		{
			...
			if (Define.UseEditorResouces)
			{
				...
				foreach (string s in realPath)
				{
                    ...
                    if (IsSprite)
                    {
                        UnityEngine.Sprite sprites = AssetDatabase.LoadAssetAtPath<UnityEngine.Sprite>(s);
                        AddSpriteResource(assetBundleName, assetName, sprites);
                    }
                    else
                    {
			...
			if (!assetBundle.isStreamedSceneAssetBundle)
			{
                if (IsSprite)
                {
                    UnityEngine.Sprite[] assets = assetBundle.LoadAllAssets<Sprite>();
                    foreach (UnityEngine.Sprite asset in assets)
                    {
                        AddSpriteResource(assetBundleName, asset.name, asset);
                    }
                }
                else
                {
